...
> ・「最適化モード」使用中は、Link 接続時のメニューは使用できません。  
...
① 通常モードボタン: Link 接続時のメニューをオンにします。(デフォルトの状態)  
② 最適化モードボタン: Link 接続時のメニューをオフにして、軽量化します。  
③ ステータス: ツールの処理内容が表示されます。  
④ サービスステータス: Link接続時に使用されるサービスの起動状態が表示されます。  

・SteamVRと有線Link接続を同時に使用する場合は「最適化モード」を使用してください。  
→接続が確認できた状態でSteamVRを起動すると、VRに画面が表示されます。

・SteamVRを使用しない場合、Link 接続時のメニューを使用したい場合は必ず「通常モード」を使用してください。  
...
> [!IMPORTANT]
> 本ツールは Meta の Link 接続時のメニューを無効化します。  
> そのため、SteamVR を起動しない場合、本来のメニュー画面が表示されず、  
> VRが正常に動作しません。  
> Link 接続時のメニューを使用したい場合は、<ins>**必ず「通常モード」に戻してください。**</ins>
